...
const int ESTATEINDEX = CARDSHELD - 1; // ESTATE CARDS ARE HELD AT THE END OF HAND
...
//                                                          //
int oneEstateBaron (int des, int passOrFail){
...
//  Player is unable to discard estate when no estates on hand  //
    ...
    printf("Testing if hand size is 3\n");
    passOrFail += conditionTester(((testState.handCount[0] - 1) == gState.handCount[0]) && (gState.handCount[0] == 3));
...
//                                                           //
    ...
    printf("Testing if hand size is 4\n");
    passOrFail += conditionTester((testState.handCount[0] == gState.handCount[0]) && (gState.handCount[0] == 4));
...
//  Test player state with "Greater than 0" estate cards    //
//                                                          //
int zeroEstateBaron(int des, int passOrFail){
    struct gameState gState, testState;
    //Iniitalize gameState object with set infomation 
    initializeGame(PLAYERCOUNT, kingCards, RANDOMSEED, &gState);
    gState.handCount[0] = CARDSHELD;
...
//  Player is unable to discard estate when no estates on hand  //
    ...
    printf("Testing if hand size is 5\n");
    passOrFail += conditionTester((testState.handCount[0] == gState.handCount[0]) && (gState.handCount[0] == 5));
...
int main (){ 
    ...
    //Try function with estate card (both choices) and baron card (both choices)
        for (des = 0; des < 2; des++)
            passOrFail += oneEstateBaron(des, 0);
    //Try function with no estate case.
        for (des = 0; des < 2; des++)
            passOrFail += zeroEstateBaron(des, 0);
